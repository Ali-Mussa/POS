...
class PembelianController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $details = PembelianDetail::with('produk')
                ->where('id_pembelian', $pembelian->id_pembelian)
                ->get();
            $supplierName = $pembelian->supplier->nama ?? 'Unknown';
            $admin = User::where('level', 1)->first();

            if ($admin) {
                Mail::to($admin->email)->send(new PurchaseNotification($pembelian, $details, $supplierName));
            }
        } catch (\Exception $e) {
